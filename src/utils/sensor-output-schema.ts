...
const field = {
  pOhms(label: string, place: string) {
    return z.coerce.number({
      error: issue => issue.input === undefined ? `${label} is missing. Put the ${place} number there.` : `${label} must be a number.`,
    });
  },
...
export const sensorOutputSchema = z.preprocess(
  ...
  z.object({
    ...
    boostVoltageV: z.coerce.number({
      error: issue => issue.input === undefined ? "Boost voltage is missing. Put it after the six status codes." : "Boost voltage must be a number.",
    }),
    boxStatusCode: z
      .string({
        error: issue => issue.input === undefined ? "Box status code is missing. Put it last, like 0x00000001." : "Box status code must be text like 0x00000001.",
      })
      ...
      .refine(value => !Number.isNaN(Number.parseInt(value.slice(2), 16)), "Box status code must be valid hex, like 0x00000001.")
