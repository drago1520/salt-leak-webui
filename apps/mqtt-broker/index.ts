...
const wsServer = Bun.serve<{ channel: string }>({
  ...
  websocket: {
    open(ws) { ws.subscribe(ws.data.channel); console.log('WS connected:', ws.data.channel); },
    close(ws) { ws.unsubscribe(ws.data.channel); console.log('WS disconnected:', ws.data.channel); },
    message() {},
  },
});

const TOPIC_RE = /^([^/]+)\/([^/]+)\/([^/]+)\/(\d+)\/(\d+)$/;
const PIN_KEYS = ['p1Ohms', 'p2Ohms', 'p3Ohms', 'p4Ohms', 'p5Ohms', 'p6Ohms'] as const;

broker.on('publish', async (packet: AedesPublishPacket, client: Client | null) => {
  if (!client) return;
  try {
    const [companyId, locationId, sensorType, datacenterIdText, machineIdText] = z.tuple([z.string(), z.string(), z.string(), z.string(), z.string()]).parse(TOPIC_RE.exec(packet.topic)?.slice(1));
    const channel = sensorChannelSchema.parse({ companyId, locationId, sensorType, datacenterId: Number(datacenterIdText), machineId: Number(machineIdText) });
    ...
    wsServer.publish(toMqttTopic(channel), JSON.stringify(event, (_, v) => typeof v === 'bigint' ? v.toString() : v));
    await db.insert(sensorReadings).values({ id, datacenterId, machineId, ...reading });
  } catch (error) {
...
createServer(broker.handle).listen(env.MQTT_PORT, () => console.log(`MQTT on :${env.MQTT_PORT}`));
